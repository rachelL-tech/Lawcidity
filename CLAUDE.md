...
> 1) 依 filters 篩選來源判決（法條 / 法院層級 / 年限 / 地圖所在地），計算「被引用最多」的判決排行（rankings），
> 2) 依 keyword 全文搜尋（可選「包含」/「不含」兩種模式），計算「被引用最多」的判決排行（rankings），
> 3)（未來）點進來源判決頁，螢光筆標示 snippet；視覺化（引用網路圖、趨勢圖）；多地圖篩選。
