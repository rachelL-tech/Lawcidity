...
*Citations: dropping global score ranking.* Citation preview originally required scoring every citation across all matched sources to determine display order — this was the single largest latency cost in citation expansion. After experimenting with multiple scoring strategies, the conclusion was clear: preview only needs to "look like a hit," not be globally ranked by relevance. The final design reuses the top 5 ranked source IDs per target already cached from the search results, and picks one representative citation per source by keyword and statute hit flags.
