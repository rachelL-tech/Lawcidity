...
キーワード検索から意味理解まで。Lawcidity は、実務上参照価値の高い裁判所の見解を見つけやすくします。
...
法律上の引用は学術論文の引用によく似ています。データ構造の観点から見ると、**PageRank** の発想にもかなり近いものです。

> ある判決が他の判決から頻繁に引用されているなら、実務上一定の重みを持っている可能性が高い。

法律事務所でのインターン中、異なる **source decisions** から同じ **target decision** を指す citation snippets には、非常によく似た文言が現れることに気づきました。

これは、引用数の多い target decision が単に「多くの判決で言及されている」だけではなく、すでに安定した法的見解を形成している可能性を示しています。異なる裁判所が類似の争点を扱うとき、同じ target を繰り返し引用し、似た文言でその法的理由を説明します。
...
- 全文検索が問うのは: 同じキーワードを **含む** 判決はどれか
- 引用ランキングが問うのは: そのキーワードに関連する法的争点を処理するために、裁判所が実際に使っている判決はどれか
...
> 利用者の検索条件に関連する中核的な法的理由付けの中で、どの target が裁判所によって繰り返し引用されているか
...
**引用展開の高速化**  
- まず OpenSearch が返した `preview source IDs` だけを候補にし、各 `source` では最も関連度の高い `citation` を 1 件だけ選んでから判決情報を補うようにした
- それ以外の引用も、各 `source` から `citation` を 1 件だけ先に取り出してから判決情報を結合する形にし、不要な `join` と `sort` を減らした

これにより、引用展開は約 `3 秒` から約 `0.8 秒` に短縮されました。

**UIに表示する値の事前計算**  
- UI に表示する事件番号や引用数はあらかじめ計算して持たせ、検索のたびに計算し直さないようにした
...
- `chunks` の境界設計を見直し、意味分割や LLM 補助分割を含めて、事実記述・当事者主張・裁判所自身の法的理由付けをより明確に分ける
- 利用者クエリをより精密な法律争点や実務用語に LLM で書き換えることで、検索再現率と関連性が改善するかを検証する
