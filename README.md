...
- ある被引用判決について、異なる引用元判決のヒット断片をまず確認し、必要に応じてさらに展開
- 引用元判決の原文をそのまま開く
...
| モード | 仕組み | 解決すること |
|---|---|---|
| **キーワード検索** | 検索条件にヒットした先行判決の引用断片をまず見つけ、その断片が共通して指している先行判決を集約して順位付けする | 単に「全文にキーワードがヒットした判決」を探すのではなく、この検索のもとで裁判所が繰り返し指している先行判決と法的見解を見つける |
| **セマンティック検索（RAG）** | 利用者の問題と引用断片周辺の法的論述との類似度を比べ、判決単位に集約して意味的に近い判決を見つける | 厳密なキーワード一致への依存を下げ、「言い方は違っても争点は同じ」裁判所の法的論述を見つける |
...
- **引用関係をランキングの仕組みの中核にしている。**  
  「検索条件にヒットした判決」を探すだけでなく、この検索に関連する争点のもとで、裁判所から繰り返し援用される先行判決と法的見解を優先して上位に出します。

- **精度だけでなく速度も最適化している。**  
  「詐欺」のようにヒット件数が多い検索でも、約 `73 秒` かかっていた検索を `2-4 秒` まで短縮し、再ランキング時も、キャッシュにヒットすれば `1 ms` 未満まで下がります。
...
- **R — Retrieval（検索・争点ごとの fan-out）**  
  確認後の各争点をそれぞれ `embedding` に変換し、争点ごとに pgvector から意味的に近い `citation` 起点の `chunks` を取得し、集約してから争点ごとにまとめて返す
...
確認された各争点を、入力全体を 1 つのベクトルに圧縮するのではなく、それぞれ独立に検索にかけます。こうすることで、生成側でも争点ごとに対応する判決根拠を示せます。

- Voyage API（`voyage-law-2`）で各争点をそれぞれ `embedding` に変換
- 争点ごとに PostgreSQL / pgvector の IVFFlat インデックスで近似検索を行い、意味的類似度で並べて `chunks` を取得
- 同じ引用断片は異なる判決にそのまま流用されることが多いため、同一内容は 1 件にまとめ、審級が最も高い判決を代表とする
- 各争点で上位数件を取り、最後に争点ごとにまとめて返す
...
### 4. 可観測性：Retrieval trace

RAG は embedding・ベクトル召回・Gemini 生成という外部または重い計算ステージを 3 つ含み、どこかが遅くなったり召回が悪化したりしても、勘に頼るべきではありません。そこで RAG リクエストごとに 1 本の trace を記録し、永続化してフロントエンドの専用ページで閲覧できるようにしています。

各 trace は 3 つのステージを含みます。
- **embedding**：選定された争点と embedding のレイテンシ
- **ベクトル召回**：争点ごとの類似度分布（最高 / 中央 / 最低）。召回の質が健全かを見る
- **集約**：争点ごとに召回した後の先行判決の分布と集中度。少数の先行判決に本当に収束しているかを見る

embedding や召回の段階で失敗しても trace は記録に残るため、失敗ケースを後から診断できます。

---

### 5. 信頼性：外部呼び出しの timeout と retry

RAG の 1 リクエストは、データベース・Voyage・Gemini の 3 つの外部呼び出しに同期的に依存します。どれか 1 つが固まっても worker を占有してサービス全体を止めるべきではないため、3 つとも timeout と retry を設定しています。timeout は retry の前提です。呼び出しが応答せず固まると、timeout が無ければ次の再試行に永遠に進めません。

| 外部呼び出し | timeout | retry |
|---|---|---|
| データベース接続 | 5s | 3 回 |
| Gemini | 60s | SDK 内蔵バックオフ（タイムアウト／429／5xx のみ） |
| Voyage embedding | 15s | SDK 内蔵バックオフ（タイムアウト／429／5xx のみ） |

---

## 開発の流れ

8 つのフェーズにわたる反復開発で、司法院の生の JSON から出発し、実際に使える検索プロダクトへと段階的に仕上げました。
...
| **8. RAG 強化と可観測性** | 5 月 14 日-6 月 6 日 | chunk 構造を集約し、RAG を争点ごとの fan-out 検索へ変更し、ベクトル召回パラメータを調整し、retrieval trace の可観測レイヤー、外部呼び出しのリトライと readiness チェック、GitHub Actions CI/CD を追加 |
...
- retrieval trace をローカルファイルから検索可能な可観測バックエンドへ拡張し、「争点抽出」と「分析生成」の 2 リクエストを紐付けて、利用者全体の流れを可視化する
