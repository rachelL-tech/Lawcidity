...
| 模式 | 怎麼做 | 解決什麼問題 |
|---|---|---|
| **關鍵字搜尋** | 先從全文召回 source，再從各自的 citation snippets 中找出共同指向的 target，依引用強度排序 | 不是只找「提到同樣詞彙」的判決，而是找「法院在討論這個法律爭點時最常援引」的判決 |
| **語意搜尋（RAG）** | 將使用者查詢與 citation-anchored chunks 向量化，依語意相似度召回 | 降低對精確關鍵字的依賴，改以法律語意找出相近案例 |
