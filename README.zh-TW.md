...
我在律師事務所實習期間觀察到：來自不同 **source decisions**、但指向同一個 **target decision** 的 citation snippets，常常會出現高度相似的文字。

這表示高引用量的 target decision 不只是「被很多判決提到」，而是可能已經形成穩定的法律見解。當不同法院處理相似爭點時，會反覆引用 target，並用相近的文字說明其法律理由。
...
> 在與使用者搜尋條件相關的核心法律論述中，哪些 target 被法院反覆引用。
...
**展開引用：兩階段載入**  
卡片展開時要顯示的 snippet 分成「初次載入」跟「查看更多」兩階段。
...
- **查看更多**：Stage 2 為每個 target 只先取 5 筆 preview source ID。
  ...
  考慮過搜尋時就補滿，但對熱門 keyword（例如損害、112k source）會多花約 `+3 秒`，
  因此改成只在命中數 > 顯示數時才出現「查看更多」按鈕，
  點擊時用 PostgreSQL 一條 SQL 回傳下一頁 5 筆 snippet。
  額外成本只在使用者實際展開的 target 才發生。
