...
從關鍵字到語意理解，Lawcidity 協助使用者快速找出具有參考價值的法院見解。
...
- 先查看 target 被不同 source 引用的命中片段，必要時可展開更多
- 直接查看 source 原文
...
引用關係如圖：一份後來作成的判決會引用一份前案，而法院引用前案時周圍的上下文，常反映實務上相對穩定的法律見解。

Lawcidity 不只是找哪些判決「出現過」使用者的查詢，而是找：

> 在這個查詢下，哪些前案實際被法院反覆引用，後來的法院又是如何把前案整理成法律見解來使用。
...
- `citation snippet` 告訴我們：後來的法院怎麼把前案濃縮成法律見解，並用來支撐自己的論述
...
| 模式 | 怎麼做 | 解決什麼問題 |
|---|---|---|
| **關鍵字搜尋** | 先篩選命中查詢的引用前案片段，再依共同指向的前案做聚合、排序 | 不只找「全文有命中關鍵字」的判決，而是找出在這個查詢下，法院反覆引用的前案與法律見解 |
| **語意搜尋（RAG）** | 比對「使用者問題」與「引用片段周圍文字」的相似度，再聚合找出語意上相近的判決 | 降低對精確關鍵字的依賴，找出「說法不同、爭點相同」的法律見解　|
...
- **以引用關係作為排名機制的核心。**  
  不是只找出「命中搜尋條件」的判決，而是優先找出在這個查詢相關的爭點下，被法院反覆援引的前案與法律見解。
...
同資料池 benchmark 下，Stage 1 全文召回的查詢延遲分別是：
...
2. **`matched_citation_count`**  
   在同一個 MSM 層級內，再看有多少不同 `source` 的命中片段共同指向同一個 `target`。

換句話說，Lawcidity 找的不是單純「全文中提到搜尋關鍵字或法條」的判決，而是：

> 在這個查詢下，哪些前案會被不同法院的命中片段反覆指向？
