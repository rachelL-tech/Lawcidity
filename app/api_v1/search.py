...
def _fmt_case_ref(jyear, jcase_norm, jno, display_title=None, auth_type=None):
    if display_title:
...
def search(req: SearchRequest):
    # Build internal query terms / statute filters from request
    try:
    ...
    with get_conn() as conn:
        ...
        # Fetch all ranked targets (no limit, paginate in Python)
        all_rows = fetch_rankings_by_source_ids(
            conn=conn,
    ...
    if req.sort == "total_citation_count":
        all_rows.sort(key=lambda x: (-(x.get("total_citation_count") or 0), -(x.get("citation_count") or 0)))
    # Default: matched_citation_count (SQL already orders by score DESC, citation_count DESC)

    total = len(all_rows)
    start = (req.page - 1) * req.page_size
    page_rows = all_rows[start:start + req.page_size]
    ...
    results = [
        SearchResultItem(
            ...
            doc_type=row.get("doc_type") or row.get("auth_type"),
