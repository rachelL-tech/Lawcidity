...
# 字別前綴改為 [\u4e00-\u9fff]，與 ANY_COURT_CITATION 一致
ABBR_CITATION = re.compile(
    r'[、，及與暨或,]\s*(\d{2,3})\s*年\s*度?\s*([\u4e00-\u9fff][^字]{0,20}?)\s*字?\s*第\s*(\d+)\s*號'
)
...
# .{0,50}：允許段號後有說明文字（如「四、上訴人對原判決上訴，主張略以：」）
# 關鍵詞擴充：主張略以／答辯略以／抗辯略以／聲請人主張／反訴主張 等多種常見形態
# ㈠㈡ 子條款格式（行政法院常見）：「㈠主張要旨：」「㈡答辯要旨：」
#   - 圓括號數字 [㈠㈡㈢㈣㈤㈥㈦㈧㈨㈩] 作為節次符號，其後可無分隔符直接接 keyword
_PARTY_SECTION_RE = re.compile(
    r'(?:[一二三四五六七八九十壹貳參肆伍陸柒捌玖甲乙丙丁]+[、：,，]'
    r'|[㈠㈡㈢㈣㈤㈥㈦㈧㈨㈩])'
    r'.{0,50}'
    r'(?:原告(?:起訴)?主張|被告(?:答辯|抗辯)?|抗告意旨|上訴意旨'
